...
(async () => {
    ...
    // a. A person with the following details: Single, 
     await application_type[0].click();

    // b. A person with the following details: 0 dependants, 
    ...
    // c. A person with the following details: buying a home to live in, 
    await property_type.click()
    
    // d. A person with the following details: with income of $80,000, 
    await your_earnings[0].click()
    await your_earnings[0].type("80000")

    // e. A person with the following details: other income $10,000, 
    await your_other_income[0].click()
    await your_other_income[0].type("10000")

    // f. A person with the following details: living expenses $500, 
    await living_expenses[0].click()
    await living_expenses[0].type("500")

    // g. A person with the following details: current home loan repayments $0,
    ...
    // i. A person with the following details: other commitments $0 
    await other_commitments[0].click();
    await other_loan_repayments[0].type("0");

    // j. A person with the following details: and total credit card limits $10,000 
